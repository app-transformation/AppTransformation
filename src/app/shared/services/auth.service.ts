...
export class AuthService {
  ...
  // For /transformation route, do not check token
  isAuthenticated() {
    return true;
  }
  ...
  loginUser(creds) {
    ...
    if (!this.config.isBackend) {
      // Mock login: save a fake token and call receiveToken
      const fakeToken = 'mock.jwt.token.' + btoa(creds.email);
      localStorage.setItem('token', fakeToken);
      this.receiveToken(fakeToken);
      this.toastr.success('Logged in (mock mode)');
      return;
    }
    if (creds.social) {
